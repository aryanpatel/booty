...
class Server
{
    ...
    protected function needsCompile($in, $out, &$etag)
    {
        ...
        if (is_readable($metadataName)) {
            ...
            $metadata = unserialize($reduxFramework->filesystem->execute('get_contents', $metadataName));
    ...
    protected function compile($in, $out)
    {
        ...
        $css     = $this->scss->compile($reduxFramework->filesystem->execute('get_contents', $in), $in);
        ...
        $reduxFramework->filesystem->execute('put_contents', $out, $css);
        $reduxFramework->filesystem->execute('put_contents', 
    ...
    public function compileFile($in, $out = null)
    {
        ...
        $compiled = $this->scss->compile($reduxFramework->filesystem->execute('get_contents', $in), $in);

        if ($out !== null) {
            return $reduxFramework->filesystem->execute('put_contents', $out, $compiled);
        }
    ...
    public function serve($salt = '')
    {
        ...
        if ($input = $this->findInput()) {
            ...
            echo $reduxFramework->filesystem->execute('get_contents', $output);
